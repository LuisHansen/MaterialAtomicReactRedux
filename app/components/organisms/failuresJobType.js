...
var FailuresJobType = React.createClass({
	componentWillMount() {
	...
	populateTable() {
		...
		this.props.drilldown.data.map((job) => {
			...
			jobs.hasOwnProperty(name) ? jobs[name] += job.failures : jobs[name] = job.failures;
		})
		let head = (<thead>
			<tr>
				<th>Job type</th>
	...
	formatData() {
		...
		this.props.drilldown.data.map((job) => {
			let name = job.name ? job.name : 'Unknown'
			legend.hasOwnProperty(name) ? legend[name] += job.failures : legend[name] = job.failures;
		})
		let labels = [];
		let data = [];
			...
			return (
				<div className="col s12 m6 l6">
					<div className="card">
						...
						<div className="card-content">
							<div id="chart-jobtype"><Graph id="failuresJobTypeChart" width="100" height="100" config={this.formatData()} /></div>
...
export default connect(mapStateToProps, mapDispatcherToProps)(FailuresJobType);