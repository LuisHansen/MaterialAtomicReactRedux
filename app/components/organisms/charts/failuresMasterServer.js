...
var FailuresMasterServer = React.createClass({
	componentWillMount() {
	...
	formatData() {
		...
		this.props.drilldown.data.map((job) => {
			legend.hasOwnProperty(job.server) ? legend[job.server] += job.failures : legend[job.server] = job.failures;
		})
		let labels = [];
		let data = [];
			...
			return (
				<div className="col s12 m6 l6">
					<Card>
						...
						<Graph id="charts/failuresMasterServerChart" width="100" height="100" config={this.formatData()} />
					</Card>
				</div>
			);
		} else {
...
export default connect(mapStateToProps, mapDispatcherToProps)(FailuresMasterServer);